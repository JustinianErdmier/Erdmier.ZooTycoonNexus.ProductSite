...
public static class IniEntryKindsExtensions
{
    ...
    /// <returns><c>"k"</c> for <see cref="IniEntryKinds.Key" />; <c>"ok"</c> for <see cref="IniEntryKinds.Ok" />; <c>"v"</c> for <see cref="IniEntryKinds.Value" />.</returns>
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="kind" /> is not a recognised value.</exception>
    public static string MapToCssToken(this IniEntryKinds kind)
        => kind switch
        {
            IniEntryKinds.Key   => "k",
            IniEntryKinds.Ok    => "ok",
            IniEntryKinds.Value => "v",
            var _               => throw new ArgumentOutOfRangeException(nameof(kind), kind, message: null)
        };
}
